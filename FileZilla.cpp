...
USE AT OWN RISK"), _T("Important Information"));
    ...
    // ------------------------------------------------------
    {
      ...
      wxString mime_icon_dir = m_resourceDir.GetPath() + "/mime/";
      std::cout << mime_icon_dir << std::endl;
      // Initialize the mime entry
      std::vector<const char *> mime_extensions = {
	"h5",
	"caffemodel", "solverstate",
	"caffemodel.h5", "solverstate.h5",
	...
	"c", "cc", "cpp", "csh", "css", 
	"h", "hpp",
	"lua", "perl", "py", "rb", "sh", "vala",
	
	"html", "htm", "php", "xml", "sgml", "xhtml",

	"es", "java", "js", "json",
	
	"7z", "bz", "bz2", "cab", "cpio", "deb", "gz",
	"jar", "lha", "lzma", "lzop", "rar", "rpm",
	"shar", "tar", "tgz", "zip", "xz", "Z",
	
	"bmp", "gif", "jpeg", "jpg", "j2k", "png", "PNG", "psd", "raw",
	"tif", "tiff", "gif", "ico", "tga", "webp", "xcf",

	"ai", "svg", "svgz",

	"aac", "flac", "mid", "midi", "mp3", "oga", "ogg", "wav", "wma",

	"3g2", "3gp", "avi", "mpeg", "mp4", "ogv", "ogx", "swf", "webm", "wmv", "yuv",
	
	"pdf",

	"docx", "doc","docm", "odt",
	"csv", "ods", "tsv", "xlsx", "xls", "xlsm",
	"pptx", "ppt","pptm", "odp"
      };
      
      for ( auto ext: mime_extensions )
      {
	   
	   wxFileType *pType = wxTheMimeTypesManager->GetFileTypeFromExtension(ext);
	   wxString mime_type;
	   if ( pType && pType->GetMimeType(&mime_type) )
	   {
		wxFileTypeInfo info(mime_type);
		info.SetIcon(mime_icon_dir + ext + ".png");
		wxTheMimeTypesManager->Associate(info);
	   }
	   else
	   {
		wxFileTypeInfo info(wxString("application/x-") + ext);
